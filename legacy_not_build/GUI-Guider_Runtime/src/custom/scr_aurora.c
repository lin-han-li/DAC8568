...
#include "../../../WifiUIStub/wifi_ui_stub.h"
 ...
 * DEFINES
 **********************/
#define AURORA_SCREEN_W 320
#define AURORA_SCREEN_H 240
#define AURORA_HEADER_H 35
#define AURORA_BODY_H (AURORA_SCREEN_H - AURORA_HEADER_H)
#define AURORA_PAGE_COUNT 3

/* ============ Performance switches ============ */
/* Background floating animation will force large invalidated areas and heavy
 * blending. */
#ifndef EW_AURORA_BG_ANIM
#define EW_AURORA_BG_ANIM 0
#endif

/* Gradients are expensive (often use RGB888 intermediate). Disable by default
 * for FPS. */
...
static void start_float_anim(lv_obj_t *obj, lv_coord_t delta_y, uint32_t time,
                             uint32_t delay) {
  if (!obj) {
    return;
  }
...
static void init_aurora_styles(void) {
  ...
  lv_style_set_radius(&style_icon_box, 10);
#if EW_AURORA_USE_SHADOW
  ...
  lv_style_set_text_font(&style_title, gui_assets_get_font_12());
  lv_style_set_text_color(&style_title, lv_color_hex(0x334155));

  styles_inited = true;
}

static lv_obj_t *create_status_item(lv_obj_t *parent, const char *text,
                                    lv_obj_t **dot_out, lv_obj_t **lbl_out,
                                    bool big_dot) {
  if (!parent) {
    return NULL;
  }
  ...
  lv_obj_set_size(dot, big_dot ? 8 : 6, big_dot ? 8 : 6);
...
static void create_aurora_background(lv_obj_t *parent) {
  ...
  lv_obj_align(blob3, LV_ALIGN_CENTER, 16, -4);
  lv_obj_clear_flag(blob3, LV_OBJ_FLAG_SCROLLABLE);
#if EW_AURORA_BG_ANIM
  start_float_anim(blob3, 5, 9000, 1400);
#endif
}
...
static void aurora_nav_event_cb(lv_event_t *e) {
  ...
  case AURORA_NAV_PARAM:
    ui_load_scr_animation(ctx->ui, &ctx->ui->ParamConfig,
                          ctx->ui->ParamConfig_del, &ctx->ui->Main_1_del,
                          setup_scr_ParamConfig, LV_SCR_LOAD_ANIM_FADE_ON, 200,
                          20, false, false);
    break;
  case AURORA_NAV_WIFI:
    ui_load_scr_animation(ctx->ui, &ctx->ui->WifiConfig,
                          ctx->ui->WifiConfig_del, &ctx->ui->Main_1_del,
                          setup_scr_WifiConfig, LV_SCR_LOAD_ANIM_FADE_ON, 200,
                          20, false, false);
    break;
  case AURORA_NAV_SERVER:
    ui_load_scr_animation(ctx->ui, &ctx->ui->ServerConfig,
                          ctx->ui->ServerConfig_del, &ctx->ui->Main_1_del,
                          setup_scr_ServerConfig, LV_SCR_LOAD_ANIM_FADE_ON, 200,
                          20, false, false);
    break;
  case AURORA_NAV_DEVICE:
    ui_load_scr_animation(ctx->ui, &ctx->ui->DeviceConnect,
                          ctx->ui->DeviceConnect_del, &ctx->ui->Main_1_del,
                          setup_scr_DeviceConnect, LV_SCR_LOAD_ANIM_FADE_ON,
                          200, 20, false, false);
    break;
  default:
    break;
...
static lv_obj_t *create_app_card(lv_obj_t *parent, const char *name,
                                 uint32_t icon_index, lv_color_t glow_color,
                                 aurora_nav_ctx_t *nav_ctx) {
...
static lv_obj_t *create_grid_page(lv_obj_t *parent) {
  ...
  lv_obj_set_flex_align(page, LV_FLEX_ALIGN_SPACE_EVENLY, LV_FLEX_ALIGN_CENTER,
...
void setup_scr_Aurora(lv_ui *ui) {
  ...
  lv_obj_set_flex_align(title_box, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  lv_obj_set_style_pad_gap(title_box, 4, 0);
  ...
  lv_obj_set_style_text_font(title_lbl, gui_assets_get_font_16(), 0);
  ...
  lv_obj_set_flex_align(status_pill, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  lv_obj_set_style_pad_hor(status_pill, 4, 0);
  lv_obj_set_style_pad_ver(status_pill, 2, 0);
  lv_obj_set_style_pad_gap(status_pill, 2, 0);
  ...
  (void)create_status_item(row1, "WIFI", &s_dot_wifi, &s_lbl_wifi,
                           true /* bigger */);
  ...
  lv_obj_set_style_text_font(node_lbl, gui_assets_get_font_12(), 0);
  s_lbl_node = node_lbl;
  ...
  lv_obj_add_event_cb(ui_Carousel, carousel_event_cb, LV_EVENT_SCROLL_END,
                      NULL);
  ...
  lv_obj_set_size(ui_PageIndicator, 100, 14);
  lv_obj_align(ui_PageIndicator, LV_ALIGN_BOTTOM_MID, 0, -3);
  lv_obj_set_flex_flow(ui_PageIndicator, LV_FLEX_FLOW_ROW);
  lv_obj_set_flex_align(ui_PageIndicator, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
  lv_obj_set_style_pad_gap(ui_PageIndicator, 6, 0);
