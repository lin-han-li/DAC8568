...
bool ew_boot_anim_is_finished(void) { return ew_boot_anim.finished; }
...
static void create_hex_background(void) {
  ...
  lv_obj_set_style_text_font(ew_boot_anim.hex_bg_label, &lv_font_montserrat_10,
                             0);
  ...
  lv_obj_set_style_opa(ew_boot_anim.hex_bg_label, LV_OPA_TRANSP,
                       0); /* 初始隐藏 */
...
static lv_obj_t *create_corner_line(lv_obj_t *parent, bool horizontal,
                                    int32_t len) {
...
static void create_corners(void) {
  ...
  lv_obj_t *tl_v =
      create_corner_line(ew_boot_anim.corner_tl, false, corner_len);
  ...
  lv_obj_set_pos(ew_boot_anim.corner_tr, SCREEN_W - margin - corner_len,
                 margin);
  ...
  lv_obj_t *tr_v =
      create_corner_line(ew_boot_anim.corner_tr, false, corner_len);
  ...
  lv_obj_set_pos(ew_boot_anim.corner_bl, margin,
                 SCREEN_H - margin - corner_len);
  ...
  lv_obj_t *bl_v =
      create_corner_line(ew_boot_anim.corner_bl, false, corner_len);
  ...
  lv_obj_set_pos(ew_boot_anim.corner_br, SCREEN_W - margin - corner_len,
                 SCREEN_H - margin - corner_len);
  ...
  lv_obj_t *br_v =
      create_corner_line(ew_boot_anim.corner_br, false, corner_len);
...
static void create_grid_lines(void) {
  ...
  const int32_t h_cap = (int32_t)(sizeof(ew_boot_anim.grid_lines_h) /
                                  sizeof(ew_boot_anim.grid_lines_h[0]));
  const int32_t v_cap = (int32_t)(sizeof(ew_boot_anim.grid_lines_v) /
                                  sizeof(ew_boot_anim.grid_lines_v[0]));
  const int32_t h_count = (((SCREEN_H + step - 1) / step) < h_cap)
                              ? ((SCREEN_H + step - 1) / step)
                              : h_cap;
  const int32_t v_count = (((SCREEN_W + step - 1) / step) < v_cap)
                              ? ((SCREEN_W + step - 1) / step)
                              : v_cap;
  ...
  for (int32_t i = 0; i < h_count; i++) {
    ...
    lv_obj_set_style_bg_color(ew_boot_anim.grid_lines_h[i], lv_color_white(),
                              0);
    ...
    lv_obj_set_style_opa(ew_boot_anim.grid_lines_h[i], LV_OPA_TRANSP,
                         0); /* 初始隐藏 */
  ...
  for (int32_t i = 0; i < v_count; i++) {
    ...
    lv_obj_set_style_bg_color(ew_boot_anim.grid_lines_v[i], lv_color_white(),
                              0);
    ...
    lv_obj_set_style_opa(ew_boot_anim.grid_lines_v[i], LV_OPA_TRANSP,
                         0); /* 初始隐藏 */
...
static void create_crt_scanline(void) {
  ...
  lv_obj_set_style_opa(ew_boot_anim.crt_scanline, LV_OPA_TRANSP,
                       0); /* 初始隐藏 */
  ...
  lv_timer_set_repeat_count(ew_boot_anim.scanline_timer,
                            160); /* 160次 * 5ms = 800ms */
...
static void create_terminal(void) {
  ...
  lv_obj_set_style_text_line_space(ew_boot_anim.terminal_label, compact ? 2 : 4,
                                   0);
  lv_obj_set_pos(ew_boot_anim.terminal_label, compact ? 10 : 40,
                 compact ? (SCREEN_H - 120) : (SCREEN_H - 180));
  lv_obj_set_style_opa(ew_boot_anim.terminal_label, LV_OPA_TRANSP, 0);

  /* 文字发光效果 */
  lv_obj_set_style_shadow_width(ew_boot_anim.terminal_label, compact ? 3 : 5,
                                0);
  lv_obj_set_style_shadow_color(ew_boot_anim.terminal_label, BOOT_COLOR_GREEN,
                                0);
  lv_obj_set_style_shadow_opa(ew_boot_anim.terminal_label, LV_OPA_50, 0);

  /* 启动日志定时器 */
  ew_boot_anim.log_timer =
      lv_timer_create(log_timer_cb, PHASE3_LOG_INTERVAL, NULL);
...
static void create_scan_overlay(void) {
  ...
  lv_obj_set_size(ew_boot_anim.scan_overlay, band_w,
                  band_h); /* 光带宽度，高度覆盖Logo */
  lv_obj_set_pos(ew_boot_anim.scan_overlay, -band_w - 10,
                 (SCREEN_H - band_h) / 2); /* 初始在屏幕左侧外 */
  ...
  lv_obj_set_style_bg_grad_color(ew_boot_anim.scan_overlay,
  ...
  /* 发光效果 */
  lv_obj_set_style_shadow_width(ew_boot_anim.scan_overlay, compact ? 20 : 40,
                                0);
  ...
  lv_obj_set_style_opa(ew_boot_anim.scan_overlay, LV_OPA_TRANSP,
                       0); /* 初始隐藏 */
...
static void create_logo_area(void) {
  ...
  const int32_t btn_w = compact ? (SCREEN_W - 100) : 168;
  const int32_t btn_h = compact ? 34 : 40;
  ...
  lv_obj_align(ew_boot_anim.logo_container, LV_ALIGN_CENTER, 0,
               logo_y); /* 为按钮留空间 */
  /* 完全清除所有可见样式 */
  ...
  lv_obj_set_flex_align(ew_boot_anim.logo_container, LV_FLEX_ALIGN_CENTER,
  ...
  /* 设置缩放中心点 */
  lv_obj_set_style_transform_pivot_x(ew_boot_anim.logo_container, lv_pct(50),
                                     0);
  lv_obj_set_style_transform_pivot_y(ew_boot_anim.logo_container, lv_pct(50),
                                     0);

  /* 风力图标 - 用 lv_line 绘制 SVG 路径 (放大1.5倍) */
  /* 创建旋转容器用于动画 */
  ew_boot_anim.icon_rotate_container =
      lv_obj_create(ew_boot_anim.logo_container);
  ...
  lv_obj_set_style_border_opa(ew_boot_anim.icon_rotate_container, LV_OPA_TRANSP,
                              0);
  ...
  /* 设置旋转中心点为容器中心 */
  lv_obj_set_style_transform_pivot_x(ew_boot_anim.icon_rotate_container,
                                     lv_pct(50), 0);
  lv_obj_set_style_transform_pivot_y(ew_boot_anim.icon_rotate_container,
                                     lv_pct(50), 0);

  lv_obj_t *icon_container = ew_boot_anim.icon_rotate_container;

  /* SVG: 风车上叶片 (坐标放大1.5倍) */
  static lv_point_precise_t blade1[] = {{36, 55}, {18, 24}, {54, 48}, {36, 55}};
  ...
  lv_obj_set_style_shadow_color(ew_boot_anim.logo_icon_line1, BOOT_COLOR_BLUE,
                                0);
  lv_obj_align(ew_boot_anim.logo_icon_line1, LV_ALIGN_CENTER, 0, 0);

  /* SVG: 风车下叶片 (坐标放大1.5倍) */
  static lv_point_precise_t blade2[] = {{36, 55}, {24, 86}, {60, 67}, {36, 55}};
  ...
  lv_obj_set_style_shadow_color(ew_boot_anim.logo_icon_line2, BOOT_COLOR_BLUE,
                                0);
  lv_obj_align(ew_boot_anim.logo_icon_line2, LV_ALIGN_CENTER, 0, 0);

  /* SVG: 心电图波形 (坐标放大1.5倍) */
  static lv_point_precise_t wave[] = {{36, 55}, {66, 55},  {78, 24},
                                      {90, 86}, {102, 55}, {114, 55}};
  ...
  lv_obj_set_style_shadow_color(ew_boot_anim.logo_icon_line3, BOOT_COLOR_BLUE,
                                0);
  ...
  lv_obj_set_style_text_font(
      ew_boot_anim.logo_text_red,
      compact ? &lv_font_montserrat_24 : &lv_font_montserrat_40, 0);
  lv_obj_set_style_text_color(ew_boot_anim.logo_text_red, BOOT_COLOR_RED, 0);
  lv_obj_set_style_text_letter_space(ew_boot_anim.logo_text_red, title_space,
                                     0);
  lv_obj_set_style_opa(ew_boot_anim.logo_text_red, LV_OPA_TRANSP,
                       0); /* 初始隐藏 */
  ...
  lv_obj_set_style_text_font(
      ew_boot_anim.logo_text_blue,
      compact ? &lv_font_montserrat_24 : &lv_font_montserrat_40, 0);
  lv_obj_set_style_text_color(ew_boot_anim.logo_text_blue, BOOT_COLOR_BLUE, 0);
  lv_obj_set_style_text_letter_space(ew_boot_anim.logo_text_blue, title_space,
                                     0);
  lv_obj_set_style_opa(ew_boot_anim.logo_text_blue, LV_OPA_TRANSP,
                       0); /* 初始隐藏 */
  ...
  lv_obj_set_style_shadow_opa(ew_boot_anim.logo_text, LV_OPA_90,
                              0); /* 增强: 80 -> 90 */
  ...
  lv_obj_set_style_text_font(
      ew_boot_anim.logo_sub,
      compact ? &lv_font_montserrat_10 : &lv_font_montserrat_14, 0);
  ...
  lv_obj_set_style_bg_color(ew_boot_anim.progress_bar, lv_color_hex(0x222222),
                            LV_PART_MAIN);
  lv_obj_set_style_bg_opa(ew_boot_anim.progress_bar, LV_OPA_COVER,
                          LV_PART_MAIN);
  lv_obj_set_style_radius(ew_boot_anim.progress_bar, 2, LV_PART_MAIN);
  lv_obj_set_style_bg_color(ew_boot_anim.progress_bar, BOOT_COLOR_BLUE,
                            LV_PART_INDICATOR);
  lv_obj_set_style_bg_opa(ew_boot_anim.progress_bar, LV_OPA_COVER,
                          LV_PART_INDICATOR);
  lv_obj_set_style_radius(ew_boot_anim.progress_bar, 2, LV_PART_INDICATOR);
  lv_obj_set_style_shadow_width(ew_boot_anim.progress_bar, compact ? 4 : 8,
                                LV_PART_INDICATOR);
  lv_obj_set_style_shadow_color(ew_boot_anim.progress_bar, BOOT_COLOR_BLUE,
                                LV_PART_INDICATOR);
  ...
  /* DAC 恢复按钮 - 初始隐藏（位于“进入系统”上方） */
  {
    ...
    /* 科技风格样式 - 加强辨识度 */
    lv_obj_set_style_bg_color(ew_boot_anim.recover_btn, lv_color_hex(0x0A1A28),
                              0);
    lv_obj_set_style_bg_opa(ew_boot_anim.recover_btn, 220, 0);
    lv_obj_set_style_border_width(ew_boot_anim.recover_btn, 3, 0);
    lv_obj_set_style_border_color(ew_boot_anim.recover_btn, BOOT_COLOR_CYAN, 0);
    lv_obj_set_style_border_opa(ew_boot_anim.recover_btn, LV_OPA_COVER, 0);
    lv_obj_set_style_radius(ew_boot_anim.recover_btn, 10, 0);
    lv_obj_set_style_shadow_width(ew_boot_anim.recover_btn, compact ? 16 : 28,
                                  0);
    lv_obj_set_style_shadow_color(ew_boot_anim.recover_btn, BOOT_COLOR_CYAN, 0);
    lv_obj_set_style_shadow_opa(ew_boot_anim.recover_btn, LV_OPA_80, 0);
    lv_obj_set_style_shadow_spread(ew_boot_anim.recover_btn, compact ? 2 : 4,
                                   0);
    /* 选中/聚焦效果：更亮的描边 + 外发光 */
    lv_obj_set_style_border_width(ew_boot_anim.recover_btn, 3,
                                  LV_STATE_FOCUSED);
    lv_obj_set_style_border_color(ew_boot_anim.recover_btn,
                                  lv_color_hex(0x00FFF0), LV_STATE_FOCUSED);
    lv_obj_set_style_outline_width(ew_boot_anim.recover_btn, 3,
                                   LV_STATE_FOCUSED);
    lv_obj_set_style_outline_pad(ew_boot_anim.recover_btn, 2, LV_STATE_FOCUSED);
    lv_obj_set_style_outline_color(ew_boot_anim.recover_btn,
                                   lv_color_hex(0x00FFF0), LV_STATE_FOCUSED);
    lv_obj_set_style_outline_opa(ew_boot_anim.recover_btn, LV_OPA_80,
                                 LV_STATE_FOCUSED);
    /* 按下效果 */
    lv_obj_set_style_bg_color(ew_boot_anim.recover_btn, BOOT_COLOR_CYAN,
                              LV_STATE_PRESSED);
    lv_obj_set_style_bg_opa(ew_boot_anim.recover_btn, LV_OPA_30,
                            LV_STATE_PRESSED);
    lv_obj_set_style_transform_scale(ew_boot_anim.recover_btn, 240,
                                     LV_STATE_PRESSED); /* 95% */
    /* 按钮文字 */
    ...
    lv_obj_set_style_text_font(ew_boot_anim.recover_btn_label, EW_FONT_CN_14,
                               0);
    lv_obj_set_style_text_color(ew_boot_anim.recover_btn_label,
                                BOOT_COLOR_WHITE, 0);
    lv_obj_set_style_text_letter_space(ew_boot_anim.recover_btn_label, 2, 0);
    /* 初始隐藏 */
  ...
  /* 科技风格样式 - 加强辨识度 */
  lv_obj_set_style_bg_color(ew_boot_anim.enter_btn, lv_color_hex(0x0A1628), 0);
  lv_obj_set_style_bg_opa(ew_boot_anim.enter_btn, 220, 0);
  lv_obj_set_style_border_width(ew_boot_anim.enter_btn, 3, 0);
  lv_obj_set_style_border_color(ew_boot_anim.enter_btn, BOOT_COLOR_BLUE, 0);
  lv_obj_set_style_border_opa(ew_boot_anim.enter_btn, LV_OPA_COVER, 0);
  lv_obj_set_style_radius(ew_boot_anim.enter_btn, 10, 0);
  lv_obj_set_style_shadow_width(ew_boot_anim.enter_btn, compact ? 16 : 28, 0);
  lv_obj_set_style_shadow_color(ew_boot_anim.enter_btn, BOOT_COLOR_BLUE, 0);
  lv_obj_set_style_shadow_opa(ew_boot_anim.enter_btn, LV_OPA_80, 0);
  lv_obj_set_style_shadow_spread(ew_boot_anim.enter_btn, compact ? 3 : 5, 0);
  /* 选中/聚焦效果：更亮的描边 + 外发光 */
  lv_obj_set_style_border_width(ew_boot_anim.enter_btn, 4, LV_STATE_FOCUSED);
  lv_obj_set_style_border_color(ew_boot_anim.enter_btn, lv_color_hex(0x66B3FF),
                                LV_STATE_FOCUSED);
  ...
  lv_obj_set_style_outline_color(ew_boot_anim.enter_btn, lv_color_hex(0x66B3FF),
                                 LV_STATE_FOCUSED);
  lv_obj_set_style_outline_opa(ew_boot_anim.enter_btn, LV_OPA_80,
                               LV_STATE_FOCUSED);
  /* 按下效果 */
  lv_obj_set_style_bg_color(ew_boot_anim.enter_btn, BOOT_COLOR_BLUE,
                            LV_STATE_PRESSED);
  lv_obj_set_style_bg_opa(ew_boot_anim.enter_btn, LV_OPA_40, LV_STATE_PRESSED);
  lv_obj_set_style_transform_scale(ew_boot_anim.enter_btn, 240,
                                   LV_STATE_PRESSED); /* 95% */
  /* 按钮文字 */
  ...
  lv_obj_set_style_text_font(ew_boot_anim.enter_btn_label, EW_FONT_CN_14, 0);
  lv_obj_set_style_text_color(ew_boot_anim.enter_btn_label, BOOT_COLOR_WHITE,
                              0);
  lv_obj_set_style_text_letter_space(ew_boot_anim.enter_btn_label, 2, 0);
  /* 初始隐藏 */
...
static void start_logo_phase(void) {
  ...
  lv_anim_set_values(&scan_move, -scan_pad,
                     SCREEN_W + scan_pad); /* 从屏幕左外到右外 */
  ...
  ew_boot_anim.glitch_count = 0;
  ew_boot_anim.glitch_timer =
      lv_timer_create(glitch_timer_cb, PHASE4_GLITCH_INTERVAL, NULL);
...
static void glitch_timer_cb(lv_timer_t *timer) {
  ...
  if (ew_boot_anim.glitch_count < PHASE4_GLITCH_COUNT) {
    ...
    if (pattern == 0) {
      /* 红色层向左偏移 */
      lv_obj_set_pos(ew_boot_anim.logo_text_red, -3,
                     (ew_boot_anim.glitch_count % 3) - 1);
      lv_obj_set_style_opa(ew_boot_anim.logo_text_red, LV_OPA_70, 0);
    } else if (pattern == 1) {
      /* 蓝色层向右偏移 */
      lv_obj_set_pos(ew_boot_anim.logo_text_blue, 3,
                     -(ew_boot_anim.glitch_count % 3 - 1));
...
static void recover_btn_click_cb(lv_event_t *e) {
  ...
  s_recover_btn_poll_timer =
      lv_timer_create(recover_btn_poll_timer_cb, 100, NULL);
}
...
static void show_enter_button_now(void) {
  ...
  if (ew_boot_anim.recover_btn) {
    ...
    lv_obj_add_event_cb(ew_boot_anim.recover_btn, recover_btn_click_cb,
  ...
  lv_obj_add_event_cb(ew_boot_anim.enter_btn, enter_btn_click_cb,
                      LV_EVENT_CLICKED, NULL);
...
static void start_zoomout_phase(void) {
  ...
  if (edgewind_ui_can_show_enter_button()) {
    ...
    if (s_enter_btn_wait_timer == NULL) {
      s_enter_btn_wait_timer =
          lv_timer_create(enter_btn_wait_timer_cb, 100, NULL);
    } else {
